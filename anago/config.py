...
class Config(object):
    ...
    # TODO: fix wording
    # Each parameter is defined by a tuple of
    #  ("description", defaultvalue, exmaplevalue, choices)
    # description: is a help message
    # defaultvalue: can be omitted (set to None)
    # examplevalue: something to put in autogenerated config. if None, defaultvalue will be used
    # typeconverter: is a converter as ConfigParser understands them: a method
    #  used to convert value to proper type (getint(), getfloat(), getboolean()).
    #  Use None if not applicable (will be string)
    # choices: list of acceptable values. put None is not necessary
    # Remember: a tuple of one item must have a comma ("item",). This is (Monty) Python
    # with his eggs.
    #
    # TODO: refactor MODEL_COMPONENTS
...
if __name__ == '__main__':
    ...
    print("# All parameters that can appear in configuration")
    for secname,(desc,params) in Config.allitems.items():
        print("SECTION: {}".format(secname))
        print("  Purpose: {}".format(desc))
        print("  Parameters: {}".format(params.keys()))
    ...
    if len(sys.argv) == 1:
        ...
        tests = [('dataset', 'train_data'), ('model', 'fc_dim'),
                 ('train_data',), ('noparam',), ('loss',)]
        for tst in tests:
            print("Get: {}".format(tst))
            if len(tst) > 1:
                val = conf.get(tst[0], tst[1])
            else:
                val = conf.get(tst[0])
            print("{}: {}={} (of type {})".format(tst, tst[0], val, type(val)))
    else:
        msg = "This is a playground for configuration API.\n" \
              "USAGE: {} /path/to/config.ini".format(myname)
        print(msg)
